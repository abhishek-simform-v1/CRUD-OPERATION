...
ascBtn.onclick = function () {
  ...
  switch (value) { // starting a switch statement based on the value parameter
    case "id": // if the value is "id", execute the following code block
      userData.sort((a, b) => { // sort the userData array using an arrow function as the sorting criteria
        if (ascBtn.classList.contains("asc")) { // if the ascBtn element has the class "asc", sort the array in ascending order
        ...
        } else { // otherwise, sort the array in descending order
          return -1;
        }
      });
    ...
    case "price":
      userData.sort((a, b) => {
        if (ascBtn.classList.contains("asc")) {
          return Number(a.price.toLowerCase()) > Number(b.price.toLowerCase()) ? -1 : 1;
        } else {
          return -1;
        }
      });
    ...
    case "catogories":
      userData.sort((a, b) => {
        if (ascBtn.classList.contains("asc")) {
          return a.catogories.toLowerCase() > b.catogories.toLowerCase() ? -1 : 1;
        } else {
          return -1;
        }
      });
